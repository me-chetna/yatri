...
import { Moon, Sun, Pencil, Check, X, MapPin, Award } from "lucide-react";
import {
  AVATAR_CHOICES,
  DEFAULT_PROFILE,
  loadProfile,
...
export default function ProfileClient() {
  ...
  const computedBadges = mounted ? getBadges(trips.length, profile.avatar) : [];
  ...
  return (
    <main>
      ...
      <div className="mx-auto mt-8 grid max-w-3xl gap-5 px-5 sm:px-6">
        ...
        <section className="rounded-[28px] border border-[color-mix(in_oklab,var(--ink)_10%,transparent)] bg-[var(--card)] p-6 shadow-sm">
          {!editing ? (
            <div className="flex items-center gap-5">
              <div className="grid h-20 w-20 shrink-0 place-items-center rounded-full bg-[var(--sand)] text-4xl animate-in fade-in zoom-in-75 duration-300">
                {profile.avatar}
              </div>
        ...
        {/* ── High Impact Big Achievement Grid Section ── */}
        {mounted && computedBadges.length > 0 && (
          <section className="rounded-[28px] border border-[color-mix(in_oklab,var(--ink)_10%,transparent)] bg-[var(--card)] p-6 shadow-xs animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center gap-2 mb-4">
              <Award className="h-4 w-4 text-[var(--primary)] animate-bounce" />
              <h2 className="text-xs font-bold uppercase tracking-wider text-[var(--muted-foreground)]">
                Passport Achievements Unlocked
              </h2>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {computedBadges.map((badge, idx) => (
                <div
                  key={idx}
                  className={`flex items-center gap-5 rounded-2xl border bg-gradient-to-br p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md ${badge.className}`}
                >
                  <div className="grid h-16 w-16 shrink-0 select-none place-items-center rounded-xl bg-[color-mix(in_oklab,var(--card)_60%,transparent)] text-4xl shadow-xs border border-white/10 dark:border-black/10">
                    {badge.emoji}
                  </div>
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 text-xs leading-normal opacity-80 font-medium">
                      {badge.description}
                    </div>
                  </div>
                </div>
